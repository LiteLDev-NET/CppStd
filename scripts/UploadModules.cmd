...
echo [INFO] Fetching CppStd to GitHub ...
echo.
...
if not exist Modules/CppStd/LiteLoader.NET.CppStd.dll (
    echo [WARNING] CppStd files no found. Pulling from remote...
    echo.
...
echo.
echo [INFO] Fetching CppStd to GitHub finished
echo.

@REM remove CppStd directory in Modules
echo [INFO] Removing Modules\CppStd
...
if "%LLNET_MODULE_NOW_STATUS%" neq "" (
    ...
    if "%LLNET_MODULE_NOW_BRANCH%" == "main" (
        git commit -m "From CppStd %LLNET_MODULE_NOW_TAG%"
        if [%2] == [release] (
            git tag %LLNET_MODULE_NOW_TAG%
        )
    ) else (
        git commit -m "From CppStd %LLNET_MODULE_NOW_TAG_LONG%"
    )
    echo.
    ...
    echo [INFO] No need to Upgrade CppStd.
    goto Finish
)
